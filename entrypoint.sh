...
main() {
  ...
  run_number="$(echo "${last_successful_run:-}" | jq -r '.run_number')"
  head_branch="$(echo "${last_successful_run:-}" | jq -r '.head_branch')"
  head_commit_id="$(echo "${last_successful_run:-}" | jq -r '.head_commit.id')"
  head_commit_tree_id="$(echo "${last_successful_run:-}" | jq -r '.head_commit.tree_id')"
  head_commit_message="$(echo "${last_successful_run:-}" | jq -r '.head_commit.message')"
  head_commit_timestamp="$(echo "${last_successful_run:-}" | jq -r '.head_commit.timestamp')"
  head_commit_author_name="$(echo "${last_successful_run:-}" | jq -r '.head_commit.author.name')"
  head_commit_author_email="$(echo "${last_successful_run:-}" | jq -r '.head_commit.author.email')"
  head_commit_committer_name="$(echo "${last_successful_run:-}" | jq -r '.head_commit.committer.name')"
  head_commit_committer_email="$(echo "${last_successful_run:-}" | jq -r '.head_commit.committer.email')"
